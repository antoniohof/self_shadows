...
public class PoseVisuallizer3D : MonoBehaviour
{
    ...
    public Vector3 rightKnee = new Vector3(0,0,0);
    public Vector3 leftKnee = new Vector3(0, 0, 0);
    public Vector3 rightFoot = new Vector3(0, 0, 0);
    public Vector3 leftFoot = new Vector3(0, 0, 0);
    ...
    void LateUpdate(){
        ...
        for (int i = 0; i < detecter.vertexCount + 1; i++) {
            ...
            */
            if (i == 25)
            {
                rightKnee.Set(detecter.GetPoseWorldLandmark(i).x, detecter.GetPoseWorldLandmark(i).y, detecter.GetPoseWorldLandmark(i).z);
            }
            if (i == 26)
            {
                leftKnee.Set(detecter.GetPoseWorldLandmark(i).x, detecter.GetPoseWorldLandmark(i).y, detecter.GetPoseWorldLandmark(i).z);
            };
            if (i == 27)
            {
                rightFoot.Set(detecter.GetPoseWorldLandmark(i).x, detecter.GetPoseWorldLandmark(i).y, detecter.GetPoseWorldLandmark(i).z);
            };
            if (i == 28)
            {
                leftFoot.Set(detecter.GetPoseWorldLandmark(i).x, detecter.GetPoseWorldLandmark(i).y, detecter.GetPoseWorldLandmark(i).z);
            };
        }
    } 
